...
# %%
@njit
def initial_sampling(a,b,c,runs, beta, gamma, omega_b_sq, omega_M_sq):
...
@njit(parallel = True)
def Evolution(a,b,c,runs,gamma, x_b, omega_b_sq):
  SINK = x_sink
  #sampling of initial conditions
  x_initial, p_initial = initial_sampling(a,b,c,runs, beta, gamma, omega_b_sq, omega_M_sq)
  t_step = 0.005
...
@njit(parallel = True)
def escape_matrix(runs, Ensembles, gamma):
  escape = np.zeros((Ensembles, runs))
  for i in prange(Ensembles):
    escape[i,:] = Evolution(a,b,c,runs,gamma, x_b, omega_b_sq)
  return escape

# %%
runs = 10000
Ensembles = 50
damping_array = np.linspace(0.01, 0.05, 5)
np.save('damping.npy', damping_array)
for i in range(len(damping_array)):
    mat = escape_matrix(runs, Ensembles, damping_array[i])
    np.save(f"escape_gamma_{i}.npy", mat)
    print(f'{(i+1)/5 * 100} % Complete')
