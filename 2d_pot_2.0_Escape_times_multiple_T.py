...
@njit
def Boltzmann_Sampling(a,b,lambda_3x,lambda_4x,lambda_4y,g,f, T, x_meta, y_meta, x_s, w_meta, runs):
  beta = 1/T
...
def Step(x, y, px, py, t_step, exp_term, x_noise, y_noise, dVdx, dVdy):
  ...
  dVdx = V_x(a,lambda_3x,lambda_4x,g,f,x,y)
  dVdy = V_y(b, lambda_4y, g,f, x,y)
...
def Evolution(a,b,lambda_3x, lambda_4x, lambda_4y, g,f, runs, gamma, x_s, y_s, x_meta, y_meta, w_meta, T, x_sink):
  #initial conditions
  x_initial, y_initial, px_initial, py_initial = Boltzmann_Sampling(a,b,lambda_3x,lambda_4x,lambda_4y,g,f, T, x_meta, y_meta, x_s, w_meta, runs)
...
T_array = np.linspace(0.8, 4, 6)
np.save('T_array.npy', T_array)
#determination of the sink arrays
x_sink = np.zeros(len(T_array))
for j in range(len(T_array)):
    T = T_array[j]
    upper = x_s + 0.1
    while equation(upper, a, lambda_3x, lambda_4x, V_s, T) * equation(x_s, a, lambda_3x, lambda_4x, V_s, T) > 0:
        upper += 0.1
    x_sink[j] = brentq(equation, x_s, upper, args=(a, lambda_3x, lambda_4x, V_s, T))
...
for j in range(len(T_array)):
    ...
    for i in range(gamma_num):
        mat = Escape_matrix(ensemble_num,a,b,lambda_3x, lambda_4x, lambda_4y, g,f, runs, damping[i], x_s, y_s, x_meta, y_meta, w_meta, T, x_sink)
        np.save(f"escape_gamma_{i}_temp_{j}.npy", mat)
        print(f'{(i+1)/gamma_num * 100} % Complete for T #{j+1}')
